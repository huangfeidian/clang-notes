...
namespace clang
{
	...
	// 对于token 主要有四种类型 一般的标识符(identifier） 关键词(keywords) 字符常量(nums string literal)
	// 和注解词(annotation)
	class Token
	{
	...
	public:
		...
		{
			return (Flags & StartOfLine) ? true : false;
		}
		...
		{
			return (Flags & LeadingSpace) ? true : false;
		}
		...
		{
			return (Flags & HasUDSuffix) ? true : false;
		}

		/// Returns true if this token contains a universal character name.
		bool hasUCN() const
		{
			return (Flags & HasUCN) ? true : false;
		}
